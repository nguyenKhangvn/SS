...
namespace Ecommerce.API.Repositories
{
    public class ReportRepository : IReportRepository
    {
        ...
        //overvỉew
        public async Task<int> GetUserCountAsync(DateTime? endDateUtc = null)
        {
            var query = _context.Users.AsQueryable();
            if (endDateUtc.HasValue)
            {
                // Assuming User.CreatedAt is stored as UTC
                query = query.Where(u => u.CreatedAt.ToUniversalTime() <= endDateUtc.Value);
            }
            return await query.CountAsync();
        }

        public async Task<int> GetProductCountAsync(DateTime? endDateUtc = null)
        {
            var query = _context.Products.AsQueryable();
            if (endDateUtc.HasValue)
            {
                // Assuming Product.CreatedAt is stored as UTC
                query = query.Where(p => p.CreatedAt.ToUniversalTime() <= endDateUtc.Value);
            }
            return await query.CountAsync();
        }

        public async Task<int> GetOrderCountAsync(DateTime? endDateUtc = null)
        {
            var query = _context.Orders.AsQueryable();
            if (endDateUtc.HasValue)
            {
                // Assuming Order.CreatedAt is stored as UTC
                query = query.Where(o => o.CreatedAt.ToUniversalTime() <= endDateUtc.Value);
            }
            return await query.CountAsync();
        }

        public async Task<decimal> GetTotalRevenueAsync(DateTime? endDateUtc = null)
        {
            var query = _context.Orders.AsQueryable();
            // Only sum completed/paid orders? Adjust based on your business logic and Order status
            query = query.Where(o => o.Status == OrderStatus.COMPLETED); // Example: filter by status
            ...
            // Ensure TotalAmount is not null and sum it
            return await query.SumAsync(o => o.TotalAmount);
        }

        public async Task<int> GetUserCountByCreationDateRangeAsync(DateTime startDateUtc, DateTime endDateUtc)
        {
            // Assuming User.CreatedAt is stored as UTC
            return await _context.Users
                .Where(u => u.CreatedAt.ToUniversalTime() >= startDateUtc && u.CreatedAt.ToUniversalTime() < endDateUtc)
                .CountAsync();
        }

        public async Task<List<Order>> GetOrdersByCreationDateRangeAsync(DateTime startDateUtc, DateTime endDateUtc)
        {
            // Assuming Order.CreatedAt is stored as UTC
            return await _context.Orders
                .Where(o => o.CreatedAt.ToUniversalTime() >= startDateUtc && o.CreatedAt.ToUniversalTime() < endDateUtc)
                // Optional: Include relevant navigation properties if needed later, but not for basic report calculations
